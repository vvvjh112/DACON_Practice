...
# for idx, row in test.iterrows():
#     try:
#         test.at[idx,'browser_count'] = dic[test.at[idx,'browser']]
#     except KeyError:
#         test.at[idx, 'browser_count'] = 0
#
# print("브라우저 거래수 합계 생성완료")
#
# mean_quality = train[['browser','quality']].groupby(['browser']).mean('quality')
# mean_quality = mean_quality.reset_index().rename(columns={'quality': '평균'})
# browser = mean_quality['browser'].unique()
# dic={}
# for idx, row in mean_quality.iterrows():
#     dic[mean_quality.at[idx,'browser']]=mean_quality.at[idx,'평균']
#
# for idx, row in train.iterrows():
#     try:
#         train.at[idx,'mean_quality'] = dic[train.at[idx,'browser']]
#     except KeyError:
#         train.at[idx, 'mean_quality'] = 0
#
# for idx, row in test.iterrows():
#     try:
#         test.at[idx,'mean_quality'] = dic[test.at[idx,'browser']]
#     except KeyError:
#         test.at[idx, 'mean_quality'] = 0
#
# print("브라우저 질 평균 생성완료")
#
# duration_browser = train[['browser','duration']].groupby(['browser']).sum('duration')
# duration_browser = duration_browser.reset_index().rename(columns={'duration': '합계'})
# browser = duration_browser['browser'].unique()
# dic={}
# for idx, row in duration_browser.iterrows():
#     dic[duration_browser.at[idx,'browser']]=duration_browser.at[idx,'합계']
#
# for idx, row in train.iterrows():
#     try:
#         train.at[idx,'duration_browser'] = dic[train.at[idx,'browser']]
#     except KeyError:
#         train.at[idx, 'duration_browser'] = 0
#
# for idx, row in test.iterrows():
#     try:
#         test.at[idx,'duration_browser'] = dic[test.at[idx,'browser']]
#     except KeyError:
#         test.at[idx, 'duration_browser'] = 0
# print("브라우저 유지시간  생성완료")
#
#
# # 국가별
...
#
# count_country = train[['country','transaction_revenue']].groupby(['country']).sum('transaction_revenue')
# count_country = count_country.reset_index().rename(columns={'transaction_revenue': '합계'})
# country = count_country['country'].unique()
# dic={}
# for idx, row in sum_country.iterrows():
#     dic[count_country.at[idx,'country']]=count_country.at[idx,'합계']
#
# for idx, row in train.iterrows():
#     try:
#         train.at[idx,'country_count'] = dic[train.at[idx,'country']]
#     except KeyError:
#         train.at[idx, 'country_count'] = 0
#
# for idx, row in test.iterrows():
#     try:
#         test.at[idx,'country_count'] = dic[test.at[idx,'country']]
#     except KeyError:
#         test.at[idx, 'country_count'] = 0
#
# print("국가별 거래 건 수 생성완료")
#
# #디바이스별 duration 시간
# duration_device = train[['device','duration']].groupby(['device']).sum('duration')
# duration_device = duration_device.reset_index().rename(columns={'duration': '합계'})
# device = duration_device['device'].unique()
# dic={}
# for idx, row in duration_device.iterrows():
#     dic[duration_device.at[idx,'device']]=duration_device.at[idx,'합계']
#
# for idx, row in train.iterrows():
#     try:
#         train.at[idx,'duration_device'] = dic[train.at[idx,'device']]
#     except KeyError:
#         train.at[idx, 'duration_device'] = 0
#
# for idx, row in test.iterrows():
#     try:
#         test.at[idx,'duration_device'] = dic[test.at[idx,'device']]
#     except KeyError:
#         test.at[idx, 'duration_device'] = 0
#
# print("디바이스 유지 시간 생성완료")
